#include "images_130X130.h"

#include <stdio.h>
#include <unistd.h>
#include "pico/stdlib.h"
#include "hardware/uart.h"          // For UART
#include "pico/multicore.h"         // For threading
...
// // Redirect standard allocators to TLSF versions
// #define malloc       tlsf_malloc_wrap
// #define free         tlsf_free_wrap
// #define realloc      tlsf_realloc_wrap
// #define memalign     tlsf_memalign_wrap

// extern uint8_t __data_start__, __data_end__;
// extern uint8_t __bss_start__, __bss_end__;
// extern uint8_t __StackLimit, __StackTop;

// #define TLSF_POOL_BYTES (465u * 1024u)   // 488 KB = 499,712 bytes

// #define TLSF_CTRL_BYTES 4096u            // usually plenty for mattconte/tlsf

// static alignas(16) uint8_t s_tlsf_ctrl[TLSF_CTRL_BYTES];
// static alignas(16) uint8_t s_pool_mem[TLSF_POOL_BYTES];

// static tlsf_t      s_tlsf      = NULL;
// static pool_t s_pool_handle = NULL;

// static uint32_t s_used_now  = 0;
// static uint32_t s_used_peak = 0;

// static inline void usage_add(size_t bs){ s_used_now += (uint32_t)bs; if (s_used_now > s_used_peak) s_used_peak = s_used_now; }
// static inline void usage_sub(size_t bs){ s_used_now -= (uint32_t)bs; }

// void tlsf_port_init(void)
// {
//     size_t need = tlsf_size();
//     if (TLSF_CTRL_BYTES < need) while (1) { /* tlsf ctrl too small */ }

//     s_tlsf = tlsf_create(s_tlsf_ctrl);
//     if (!s_tlsf) while (1) { /* create failed */ }

//     s_pool_handle = tlsf_add_pool(s_tlsf, s_pool_mem, TLSF_POOL_BYTES);
//     if (!s_pool_handle) while (1) { /* add pool failed */ }

//     s_used_now = s_used_peak = 0;
// }

// void tlsf_port_deinit(void)
// {
//     if (s_pool_handle) { tlsf_remove_pool(s_tlsf, s_pool_handle); s_pool_handle = NULL; }
//     if (s_tlsf)        { tlsf_destroy(s_tlsf); s_tlsf = NULL; }
//     s_used_now = s_used_peak = 0;
// }

// void* tlsf_malloc_wrap(size_t size)
// {
//     void* p = tlsf_malloc(s_tlsf, size);
//     if (p) usage_add(tlsf_block_size(p));
//     return p;
// }

// void* tlsf_realloc_wrap(void* ptr, size_t size)
// {
//     if (!ptr) {
//         void* p = tlsf_malloc(s_tlsf, size);
//         if (p) usage_add(tlsf_block_size(p));
...
//     }
//     if (size == 0) {
//         usage_sub(tlsf_block_size(ptr));
//         tlsf_free(s_tlsf, ptr);
//         return NULL;
//     }
//     size_t old_bs = tlsf_block_size(ptr);
//     void* np = tlsf_realloc(s_tlsf, ptr, size);
//     if (np) {
//         size_t new_bs = tlsf_block_size(np);
//         if (new_bs > old_bs) usage_add(new_bs - old_bs);
//         else                 usage_sub(old_bs - new_bs);
//     }
//     return np;
// }

// void* tlsf_memalign_wrap(size_t align, size_t size)
// {
//     void* p = tlsf_memalign(s_tlsf, align, size);
//     if (p) usage_add(tlsf_block_size(p));
//     return p;
// }

// void tlsf_free_wrap(void* ptr)
// {
//     if (!ptr) return;
//     usage_sub(tlsf_block_size(ptr));
//     tlsf_free(s_tlsf, ptr);
...
// uint32_t tlsf_pool_size_bytes(void)     { return TLSF_POOL_BYTES; }
// uint32_t tlsf_current_used_bytes(void)  { return s_used_now; }
// uint32_t tlsf_peak_used_bytes(void)     { return s_used_peak; }

// typedef struct { size_t total_free, largest_free; } walk_acc_t;
// static void walk_cb(void* ptr, size_t size, int used, void* user)
// {
//     (void)ptr;
//     walk_acc_t* acc = (walk_acc_t*)user;
//     if (!used) {
//         acc->total_free += size;
//         if (size > acc->largest_free) acc->largest_free = size;
//     }
// }

// void tlsf_walk_stats(uint32_t* total_free_bytes, uint32_t* largest_free_block_bytes)
// {
//     walk_acc_t acc = {0, 0};
//     tlsf_walk_pool(s_pool_handle, walk_cb, &acc);
//     if (total_free_bytes)         *total_free_bytes = (uint32_t)acc.total_free;
//     if (largest_free_block_bytes) *largest_free_block_bytes = (uint32_t)acc.largest_free;
// }

// static inline uint32_t data_size(void) {
//     return (uint32_t)(&__data_end__ - &__data_start__);
// }
...
//     return (uint32_t)(&__StackTop - &__StackLimit);
// }
...
//     // TLSF-specific
//     uint32_t tlsf_total  = tlsf_pool_size_bytes();
//     uint32_t tlsf_used   = tlsf_current_used_bytes();
//     uint32_t tlsf_peak   = tlsf_peak_used_bytes();

//     uint32_t tlsf_free = 0, tlsf_largest = 0;
//     tlsf_walk_stats(&tlsf_free, &tlsf_largest);

//     // Report
//     printf("\n=== SRAM Usage Breakdown ===\n");
//     printf("  .data:                     %u bytes\n", data);
//     printf("  .bss:                      %u bytes\n", bss);
//     printf("  Stack used (now):          %u bytes\n", stk_u);
//     printf("  Stack capacity (linker):   %u bytes\n", stk_T);

//     printf("  TLSF heap total:           %u bytes\n", tlsf_total);
//     printf("  TLSF used (current):       %u bytes\n", tlsf_used);
//     printf("  TLSF peak usage:           %u bytes\n", tlsf_peak);
//     printf("  TLSF free (walked):        %u bytes\n", tlsf_free);
//     printf("  TLSF largest free block:   %u bytes\n", tlsf_largest);

//     printf("  -----------------------------\n");
//     uint32_t est_used_now = data + bss + stk_u + tlsf_used;
//     printf("  Estimated total used now:  %u bytes\n", est_used_now);
//     printf("==============================\n\n");
...
// // Instead of logf, rename to debug_log or uart_log
...
void print_sram_usage(void) {
    ...
    // uart_log("SRAM usage breakdown (bytes):\n");
    // uart_log("  .data:                 %u\n", data);
    // uart_log("  .bss:                  %u\n", bss);
    // uart_log("  Heap used:             %u\n", heap_u);
    // uart_log("  Heap total region:     %u\n", heap_T);
    // uart_log("  Stack used (current):  %u\n", stk_u);
    // uart_log("  Stack capacity:        %u\n", stk_T);
    // uart_log("  Free (heap<->stack):   %u\n", gap);
    // uart_log("  ----\n  Estimated used now:    %u\n", data + bss + heap_u + stk_u);
...
void setup_uart0() {
    ...
    // uart_init(DEBUG_UART, BAUD_RATE);
    // gpio_set_function(UART0_TX_PIN, GPIO_FUNC_UART);
    // gpio_set_function(UART0_RX_PIN, GPIO_FUNC_UART);
    // stdout = DEBUG_UART;  // Redirect printf() to UART0
    stdio_uart_init_full(DEBUG_UART, BAUD_RATE, UART0_TX_PIN, UART0_RX_PIN);
    // critical_section_init(&log_cs);
    uart_puts(DEBUG_UART,"Setup UART0!\n");
}
...
int read_from_ring_buffer(uint8_t* out_byte) {
    printf("head: %zu, tail: %zu\n", head, tail);
    if (head == tail) return 0;  // empty
    *out_byte = ring_buffer[tail];
...
void setup_uart1() {
    ...
    // stdio_uart_init_full(SENSOR_UART, SENSOR_BAUD, UART1_TX_PIN, UART1_RX_PIN);
    uart_init(SENSOR_UART, SENSOR_BAUD);  // SENSOR_UART = uart1
    // Set pin UART1_TX_PIN for uart-tx(transmit) communication
    gpio_set_function(UART1_TX_PIN, GPIO_FUNC_UART);
    uart_puts(DEBUG_UART,"Set function for UART1 TX pin\n");
    gpio_set_function(UART1_RX_PIN, GPIO_FUNC_UART);
    uart_puts(DEBUG_UART,"Set function for UART1 RX pin\n");

    // Enable pull-up resistor for UART1 RX pin
    // RX pins are often left disconnected when no device is transmitting data; floating pin can pick up noise
    // Enabling pull-up resistor helps to keep the pin at a known state when no data is being received, preventing RX from picking up noise
    gpio_pull_up(UART1_RX_PIN);
    uart_puts(DEBUG_UART, "Enabled pull-up resistor for UART1 RX pin\n");

    uart_set_format(SENSOR_UART, 8, 1, UART_PARITY_NONE);
    uart_set_fifo_enabled(SENSOR_UART, true);  // Enable 32-byte FIFO

    // Enable RX interrupt for UART1
    irq_set_exclusive_handler(UART1_IRQ, on_uart_rx);  // ✅ Hook up handler
    irq_set_enabled(UART1_IRQ, true);
    uart_set_irq_enables(SENSOR_UART, true, false);  // Enable RX interrupt only
}

void send_uart_data(uint8_t *data, size_t len) {
    // uart_write_blocking(SENSOR_UART, data, len);

    for(size_t i=0;i<len;i++)
    {
        printf("Sending: %02X\n", data[i]);
        uart_putc(SENSOR_UART, data[i]);
    }
}

int read_uart_data_blocking(uint8_t *buffer, size_t len, int timeout) {
    memset(buffer, 0, 10000);
    // sleep_ms(1000);  // Wait for the sensor to be ready
    // uart_read_blocking(SENSOR_UART, buffer, len);
    clock_t start = clock();
    ...
    // int total = 0;
    // while (total < 199) {
    //     uart_read_blocking(SENSOR_UART, buffer + total, 10);
    //     for(size_t i=0;i<10;i++)
    //     {
    //         // buffer[i]=uart_getc(SENSOR_UART);
    //         printf("buffer[i]: %02X\n", total+i,buffer[i]);
    //     }
    ...
    while(elapsed_seconds < timeout && k<len)
    {
        // printf("k: %d; ",k);
        // buffer[k]=uart_getc(SENSOR_UART);
        // printf("head: %zu, tail: %zu\n", head, tail);
        read_from_ring_buffer(&buffer[k]);
        // printf("buffer[%d]: %02X\n", k, buffer[k]);
        k++;
        elapsed_seconds = (float)(clock() - start) / CLOCKS_PER_SEC;
    }

    // while(k<50)
    // {
    //     // Check if data is available
    //     if(uart_is_readable(SENSOR_UART))
    //     {
    //         printf("k: %d; ",k);
    //         // buffer[k]=uart_getc(SENSOR_UART);
    //         read_from_ring_buffer(&buffer[k]);
...
int read_uart_data(uint8_t *buffer, size_t len, int timeout) {
    memset(buffer, 0, 10000);
    sleep_ms(5000);  // Wait for the sensor to be ready
    // uart_read_blocking(SENSOR_UART, buffer, len);
    clock_t start = clock();
    float elapsed_seconds = 0;
    int k=0;
...
// void init_gpio() {
//     gpio_init(FINGER_WAKE_PIN);
//     // Configure this pin as input for pico2, which will hold the output from fingerprint sensor
//     gpio_set_dir(FINGER_WAKE_PIN, GPIO_IN);

//     // Doing gpio_put(FINGER_RST_PIN, 0) and then gpio_put(FINGER_RST_PIN, 1) will reset the fingerprint sensor
//     // This is the pin that will be used to reset the fingerprint sensor
//     gpio_init(FINGER_RST_PIN);
//     gpio_set_dir(FINGER_RST_PIN, GPIO_OUT);
//     gpio_put(FINGER_RST_PIN, 1);
// }


// command_buf is of length 5
uint8_t TxAndRxCmd(uint8_t* command_buf, int timeout)
{
    ...
    for(int i=0;i<5;i++)
    {
        tx_buf[i+1]=command_buf[i];
        CheckSum^=command_buf[i];
    }
    tx_buf[6]=CheckSum;
    tx_buf[7]=CMD_TAIL;

    uart_puts(DEBUG_UART,"Send tx_buf: \n");
    for(int i=0;i<8;i++)
    {
        // uart_putc(DEBUG_UART, tx_buf[i]);
        printf("%02X ", tx_buf[i]);
    }
    uart_puts(DEBUG_UART,"\n");

    send_uart_data(tx_buf, 8);  // Send command
    int read_count=read_uart_data(g_rx_buf, 8,timeout);  // Receive response

    uart_puts(DEBUG_UART,"Received g_rx_buf: \n");
    for(int i=0;i<8;i++)
    {
...
// command_buf is of length 5
uint8_t TxAndRxCmd_read_values(uint8_t* command_buf, int timeout, int len_read)
{
    memset(ring_buffer, 0, RING_BUFFER_SIZE);
    head=0; tail=0;
    ...
    uart_puts(DEBUG_UART,"Received g_rx_buf(data head): \n");
    for(int i=0;i<8;i++)
    {
        printf("%02X ", g_rx_buf[i]);
    }
    ...
    {
        read_uart_data_blocking(g_rx_buf, len_read,timeout*10);  // Receive response
        // uart_puts(DEBUG_UART,"read_count: ");
        // printf("%d\n", read_count);
        // uart_puts(DEBUG_UART,"\n");

        // if(read_count!=len_read)
        //     return ACK_TIMEOUT;

        uart_puts(DEBUG_UART,"Received g_rx_buf(data); fingerprint pixels are: \n");
        for(int i=1;i<len_read-2;i++)
        {
            printf("%02X, ", g_rx_buf[i]);
        }
        uart_puts(DEBUG_UART,"\n");
        ...
        if(checksum!=g_rx_buf[len_read-2])
        {
            printf("Checksum failed: %02X != %02X\n", checksum, g_rx_buf[197]);
            return ACK_FAIL;
        }
    }
...
// command_buf is of length 5
// command_buf_2 is of length 197
uint8_t TxAndRxCmd_download_eigen_values(uint8_t* command_buf, uint8_t* command_buf_2, int timeout, int len_read)
{
    memset(ring_buffer, 0, RING_BUFFER_SIZE);
    head=0; tail=0;
    ...
    uint8_t tx_buf_2[199]={0};
    tx_buf_2[0]=CMD_HEAD;
    for(int i=0;i<196;i++)
    {
        tx_buf_2[i+1]=command_buf_2[i];
        CheckSum^=command_buf_2[i];
    }

    tx_buf_2[197]=CheckSum;
    tx_buf_2[198]=CMD_TAIL;

    uart_puts(DEBUG_UART,"Eigen values of Database user being sent to sensor are: \n");
    for(int i=4;i<197;i++)
    {
        // uart_putc(DEBUG_UART, tx_buf[i]);
        printf("%02X ", tx_buf_2[i]);
    }

    uint8_t tx_buf_3[207]={0};
    for(int i=0;i<8;i++)
    {
        tx_buf_3[i]=tx_buf[i];
    }
    for(int i=8;i<207;i++)
    {
        tx_buf_3[i]=tx_buf_2[i-8];
    }

    for(size_t i=0;i<8;i++)
    {
        printf("Sending: %02X\n", tx_buf_3[i]);
        uart_putc(SENSOR_UART, tx_buf_3[i]);
        sleep_ms(10);
    }

    sleep_ms(1000);                // Wait for the sensor to be ready after first command

    for(size_t i=8;i<207;i++)
    {
        printf("Sending: %02X\n", tx_buf_3[i]);
        uart_putc(SENSOR_UART, tx_buf_3[i]);
        sleep_ms(10);
    ...
    // send_uart_data(tx_buf, 8);  // Send command
    // sleep_ms(1000);                // Wait for the sensor to be ready after first command
    // // send_uart_data(tx_buf_2, 199);  // Send command
    // for(size_t i=0;i<199;i++)
    // {
    //     printf("Sending: %02X\n", tx_buf_2[i]);
    //     uart_putc(SENSOR_UART, tx_buf_2[i]);
    //     sleep_ms(1);
    // }

    int read_count=read_uart_data(g_rx_buf, 8,timeout*10);  // Receive response
    if(read_count!=8)
        return ACK_TIMEOUT;
    if(g_rx_buf[0] != CMD_HEAD)
        return ACK_FAIL;
...
// {
//     uint8_t command_buf[5]={CMD_COM_LEV,0,0,1,0};
//     uint8_t r=TxAndRxCmd(command_buf);
//     if(r==ACK_TIMEOUT)
//         return ACK_TIMEOUT;
...
{
    // printf("In SetCompareLevel: %d\n", level);
    uint8_t command_buf[5]={CMD_COM_LEV,0,level,0,0};
    uint8_t r=TxAndRxCmd(command_buf,1);
    // printf("r: %x\n",r);
    // printf("g_rx_buf[4]: %x, g_rx_buf[3]: %x\n",g_rx_buf[4],g_rx_buf[3]);
...
// Just considering lower 8 bits for number of users - # of users is 0-255
uint8_t GetUserCount()
{
    uint8_t command_buf[5]={CMD_USER_CNT,0,0,0,0};
    uint8_t r=TxAndRxCmd(command_buf,1);
    if(r==ACK_TIMEOUT)
        return ACK_TIMEOUT;
    if(r==ACK_SUCCESS && g_rx_buf[4]==ACK_SUCCESS)
...
// uint8_t GetTimeOut()
// {
//     uint8_t command_buf[5]={CMD_TIMEOUT,0,0,1,0};
//     uint8_t r=TxAndRxCmd(command_buf);
//     if(r==ACK_TIMEOUT)
//         return ACK_TIMEOUT;
//     if(r==ACK_SUCCESS && g_rx_buf[4]==ACK_SUCCESS)
...
uint8_t AddUser()
{
    uint8_t r=GetUserCount();
    if(r>=USER_MAX_CNT)
        return ACK_FULL;

    uint8_t command_buf[5]={CMD_ADD_1,0,r+1,3,0};
    r=TxAndRxCmd(command_buf,6);
    if(r==ACK_TIMEOUT)
        return ACK_TIMEOUT;
    if(r==ACK_SUCCESS && g_rx_buf[4]==ACK_SUCCESS)
    {
        uart_puts(DEBUG_UART,"Success 1; Please lift and press again!\n");
        sleep_ms(2000);  
        command_buf[0]=CMD_ADD_2;
        uint8_t r_2=TxAndRxCmd(command_buf,6);
        if(r_2==ACK_TIMEOUT)
            return ACK_TIMEOUT;
        if(r_2==ACK_SUCCESS && g_rx_buf[4]==ACK_SUCCESS)
        {
            uart_puts(DEBUG_UART,"Success 2; One more time, same finger!\n");
            sleep_ms(2000);

            for(int k=0;k<2;k++)
            {
                uart_puts(DEBUG_UART,"Trying final fingerprint enrollment...\n");
                command_buf[0]=CMD_ADD_3;
                uint8_t r_3=TxAndRxCmd(command_buf,6);
                if(r_3==ACK_TIMEOUT)
                    return ACK_TIMEOUT;
                if(r_3==ACK_SUCCESS && g_rx_buf[4]==ACK_SUCCESS)
                {
                    uart_puts(DEBUG_UART,"Success 3; Fingerprint enrolled!\n");
                    return ACK_SUCCESS;
                }
            }
        }
...
uint8_t ClearAllUsers()
{
    uint8_t command_buf[5]={CMD_DEL_ALL,0,0,0,0};
    uint8_t r=TxAndRxCmd(command_buf,5);
    if(r==ACK_TIMEOUT)
        return ACK_TIMEOUT;
    if(r==ACK_SUCCESS && g_rx_buf[4]==ACK_SUCCESS)
        return ACK_SUCCESS;
...
uint8_t IsMasterUser(uint8_t user_id)
{
...
uint8_t getImageForFinger()
{
    uint8_t command_buf[5]={CMD_ACQUIRE_IMAGE_AND_UPLOAD,0,0,0,0};
    uint8_t r=TxAndRxCmd_read_values(command_buf,15,3203);
    return r;
}

uint8_t VerifyUser()
{
    uint8_t command_buf[5]={CMD_MATCH,0,0,0,0};
    uint8_t r=TxAndRxCmd(command_buf,5);
    if(r==ACK_SUCCESS && IsMasterUser(g_rx_buf[4]))
    {
        printf("Verified User ID is: %02x\n",g_rx_buf[3]);

        uint8_t command_buf_read_image[5]={CMD_ACQUIRE_IMAGE_AND_UPLOAD,0,0,0,0};
        uint8_t r=TxAndRxCmd_read_values(command_buf_read_image,15,3203);
        return r;
    }
    else if(g_rx_buf[4]==ACK_NO_USER)
        return ACK_NO_USER;
    else if(g_rx_buf[4]==ACK_TIMEOUT)
        return ACK_TIMEOUT;
    else
        return ACK_GO_OUT;
}

uint8_t getEigenValuesForFinger()
{
    uint8_t command_buf[5]={CMD_GET_EIGEN_VALUES_OF_FINGER,0,0,0,0};
    uint8_t r=TxAndRxCmd_read_values(command_buf,15,199);
    return r;
}

uint8_t compareEigenValuesForFinger()
{
    // uint8_t command_buf[5]={CMD_COMPARE_EIGEN_VALUES_OF_FINGER,0,196,0,0};
    uint8_t command_buf[5]={CMD_COMPARE_EIGEN_VALUES_WITH_ALL_USERS,0,196,0,0};
    uint8_t command_buf_2[196]; command_buf_2[0]=0; command_buf_2[1]=0; command_buf_2[2]=0;
    for(int i=3;i<196;i++)
    {
        command_buf_2[i]=fingerprint_eigen_database[i-3];
    }
    uint8_t r=TxAndRxCmd_download_eigen_values(command_buf,command_buf_2,15,199);
    return r;
}

// // Function to send command and receive response
// void send_command() {
//     uart_write_blocking(UART_ID, cmd, 8);  // Send 8-byte command
//     uart_read_blocking(UART_ID, response, 8);  // Receive 8-byte response
// }

uint8_t Analysis_PC_Command(char* command)
{
    ...
    {
        uart_puts(DEBUG_UART,"Add fingerprint (Put your finger on sensor until successfully/failed information returned)\n");
        
        uint8_t r=AddUser();
        if(r==ACK_SUCCESS)
        {
            uart_puts(DEBUG_UART,"Fingerprint added successfully.\n");
        }
        else if(r==ACK_TIMEOUT)
        {
            uart_puts(DEBUG_UART,"Timeout occurred while adding fingerprint.\n");
        }
        else if(r==ACK_FULL)
        {
            uart_puts(DEBUG_UART,"Fingerprint storage is full.\n");
        }
        else
        {
            uart_puts(DEBUG_UART,"Failed: Please try to place the center of the fingerprint flat to sensor, or this fingerprint already exists!\n");
        }
    }
    ...
    {
        int8_t result=ClearAllUsers();
        if(result==ACK_SUCCESS)
            printf("All fingerprints have been cleared !\n");
        else
            printf("Failed to clear all fingerprints !\n");
    }
    else if(strcmp(command,"CMD5")==0)
    {
        int8_t result=getImageForFinger();
        if(result==ACK_SUCCESS)
            printf("Getting image pixels for finger succeeded!\n");
        else
            printf("Failed to get image for finger!\n");
    }
    // else if(strcmp(command,"CMD5")==0)
    // {
    //     uint8_t r=compareEigenValuesForFinger();
    //     if(r==ACK_SUCCESS)
    //     {
    //         printf("Matching successful.\n");
    //     }
...
static int decode_png_file_to_argb32_javaish(ArgbImage* out, int k) {
    ...
    #define printf printf
    printf("Before argb\n");
    uint32_t* argb = (uint32_t*)malloc(n * sizeof(float));
    printf("After argb\n");
    #define printf(...)
    ...
    for (size_t i = 0; i < n; i++) {
        uint8_t g1 = lut[images[k][i]];
        argb[i]=0xFF000000u | (g1<<16) | (g1<<8) | g1;
...
////////////////////////////
void print_total_size_bytes_FeatureTemplate(const FeatureTemplate* template_final) {
    #undef printf
    #define printf printf
    printf("In total_size_bytes_used\n");

    if (!template_final) {
        printf("template_final is NULL.\n");
        return;
    }

    size_t struct_size = sizeof(*template_final);  // Shallow struct size
    size_t buffer_size = 0;

    if (template_final->minutiae.data != NULL && template_final->minutiae.capacity > 0) {
        buffer_size = (size_t)template_final->minutiae.capacity * sizeof(FeatureMinutia);
    }

    size_t total_size = struct_size + buffer_size;

    printf("Total memory used (bytes) = %zu\n", total_size);
    #define printf(...)
}
...
FeatureTemplate* extract(DoubleMatrix raw, int dpi)
{
    ...
    // HistogramCube histogram = create_histogram_cube_from_point(blocks.primary.blocks, 64);
    ...
    #undef printf
    #define printf printf
    printf("After Freed histogram.counts\n");
    print_sram_usage();
    sleep_ms(8000); 
    #define printf(...)
    ...
    printf("After equalized and smoothHistogram:\n");
    print_sram_usage();
    sleep_ms(8000); 
    #define printf(...)
    ...
    #define printf printf
    // print_histogram_cube(&smoothHistogram);
    printf("After freeing smoothHistogram:\n");
    print_sram_usage();
    sleep_ms(8000); 
    #define printf(...)
    ...
    #undef printf
    #define printf printf
    printf("Total size of template_final:\n");
    print_total_size_bytes_FeatureTemplate(template_final);
...
EdgeShape* create_edge_shape(SearchMinutia reference, SearchMinutia neighbor) {
    ...
    printf("Before POLAR arrays init\n");
    // print_sram_usage();
    // sleep_ms(8000);

    int* POLAR_DISTANCE_CACHE = (int*)(malloc(POLAR_CACHE_SIZE*sizeof(int)));
    float* POLAR_ANGLE_CACHE = (float*)(malloc(POLAR_CACHE_SIZE*sizeof(float)));

    printf("After POLAR arrays init\n");
    // print_sram_usage();
    // sleep_ms(8000);

    for(int i=0;i<POLAR_CACHE_SIZE;i++)
    {
        POLAR_DISTANCE_CACHE[i]=0;
        POLAR_ANGLE_CACHE[i]=0.0f;
    }
    ...
    free(POLAR_DISTANCE_CACHE);
    free(POLAR_ANGLE_CACHE);
...
NeighborEdge** build_table(const SearchMinutia* minutiae, int count, int* edge_count) {
    ...
    #define printf printf
    printf("count: %d\n",count);
    printf("Before edges, sq_distances, star creation\n");
    // print_sram_usage();
    // sleep_ms(8000);
    ...
    printf("After edges, sq_distances, star creation\n");
    // print_sram_usage();
    // sleep_ms(8000);
    ...
    for (int reference = 0; reference < count; ++reference) {
        ...
        printf("edge_count[%d]=star_size=%d\n",reference,edge_count[reference]);

        for(int i=0;i<star_size;i++)
        {
            printf("%f ",edges[reference]->shape->referenceAngle);
        }
        printf("\n");

        printf("After %dth iteration of build_table: ",reference);
        // print_sram_usage();
        // sleep_ms(500);
    }
...
SearchTemplate* SearchTemplate_new(const FeatureTemplate* features) {
    ...
    #undef printf
    #define printf printf
    printf("Before free(features->minutiae.data)\n");
    print_sram_usage();
    sleep_ms(8000);
    #define printf(...)
    ...
    for(int i=0;i<st->minutiae_count;i++)
    {
        free(features->minutiae.data);
    }

    // #undef printf
    // #define printf printf
    // printf("After free(features->minutiae.data)\n");
    // print_sram_usage();
    // sleep_ms(8000);
    // #define printf(...)
...
int main()
{
    ...
    for(int i=0;i<IMG_COUNT;i++)
    {
        ...
        // get_filename_from_log(image_names[i], clean_query_filename, sizeof(clean_query_filename));
        // get_representation(clean_query_filename, query_rep, sizeof(query_rep));

        // // --- Aggregate scores per candidate representation ---
        // RepScore *repScores = (RepScore*)calloc((size_t)IMG_COUNT-1, sizeof(RepScore));
        // if (!repScores) {
        ...
        #undef printf
        #define printf printf
        printf("After featureTemplate extraction: \n");
        #define printf(...)
        print_sram_usage();
        sleep_ms(8000);
        ...
        // printf("Immediately after SearchTemplate_new\n");
        // print_sram_usage();
        // sleep_ms(8000);
        // #define printf(...)

        // free(featureTemplate->minutiae.data);
        // free(featureTemSearchTplate);
        ...
        #define printf printf
        print_search_template(inner);
        printf("Immediately after build_edge_map\n");
        // print_sram_usage();
        // sleep_ms(8000);
        #define printf(...)
        ...
        #undef printf
        #define printf printf
        printf("After probe_creation: \n");
        // print_sram_usage();
        // sleep_ms(8000);
        #define printf(...)

        // free(inner->minutiae);
        // free(inner->edge_counts);
        // for(int h=0;h<inner->minutiae_count;h++)
        // {
        //     free(inner->edges[h]->shape);
        //     free(inner->edges[h]);
        // }
        // free(inner->edges);
        // free(inner);
        ...
        char* queryClass = get_class_name(image_names[i]);       
        ...
        for (int j = 0; j < IMG_COUNT; j++) {
            ...
            // candidate_decoded.data=img_2.argb;

            #undef printf
            #define printf printf
            printf("candidate_decoded.data created\n");
            #define printf(...)
            ...
            // get_filename_from_log(image_names[j], clean_cand_filename, sizeof(clean_cand_filename));
